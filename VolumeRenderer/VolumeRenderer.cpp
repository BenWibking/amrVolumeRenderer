...
std::pair<float, float> computeBoxScalarRange(
    const AmrBox& box,
  ...
  amrex::ReduceOps<amrex::ReduceOpMin, amrex::ReduceOpMax> reduceOps;
  amrex::ReduceData<float, float> reduceData(reduceOps);
  reduceOps.eval(
      totalCells,
      reduceData,
      [=] AMREX_GPU_DEVICE(
          amrex::Long linearIndex) noexcept -> amrex::GpuTuple<float, float> {
        ...
        const int localI = static_cast<int>(
            remainder -
            static_cast<amrex::Long>(localJ) * static_cast<amrex::Long>(nx));
        const int i = lo[0] + localI;
        const int j = lo[1] + localJ;
        const int k = lo[2] + localK;
        const float value =
            applyScalarTransform(values(i, j, k, component), scalarTransform);
        return {value, value};
      });

  const auto reduced = reduceData.value(reduceOps);
...
auto VolumeRenderer::loadPlotFileGeometry(
  ...
  levelGeometry.reserve(static_cast<std::size_t>(maxLevel) + 1);
  const auto probLo = plotfile.probLo();
  for (int level = 0; level <= maxLevel; ++level) {
    amrVolumeRenderer::detail::LevelGridGeometry grid;
    grid.probLo = probLo;
    grid.cellSize = plotfile.cellSize(level);
    levelGeometry.push_back(grid);
  }

  SceneGeometry scene = amrVolumeRenderer::detail::BuildSceneGeometry(
      ownedLevels,
      levelGeometry,
...
std::pair<float, float> VolumeRenderer::computeGlobalScalarRange(
  ...
  for (const auto& box : geometry.localBoxes) {
    const auto [boxMin, boxMax] =
        computeBoxScalarRange(box, geometry.scalarTransform);
    localMin = std::min(localMin, boxMin);
    localMax = std::max(localMax, boxMax);
  }
...
auto VolumeRenderer::computeScalarHistogram(
  ...
  SceneGeometry geometry = loadPlotFileGeometry(plotfilePath,
                                                ...
                                                requestedMaxLevel,
                                                logScaleInput);
  return amrVolumeRenderer::detail::ComputeSceneHistogram(geometry, binCount);
}
...
int VolumeRenderer::renderScene(
  ...
  const std::pair<float, float> scalarRange =
      geometry.hasScalarRange ? geometry.scalarRange
                              : computeGlobalScalarRange(geometry);
